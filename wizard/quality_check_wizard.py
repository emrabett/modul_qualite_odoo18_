...
class QualityCheckWizard(models.TransientModel):
    _name = 'quality.check.wizard1'
    _description = "Assistant pour Contrôle Qualité"

    check_ids = fields.Many2many('quality.check1', required=True)
    current_check_id = fields.Many2one('quality.check1', required=True)
    ...
    # Champs liés au current_check_id
    potential_failure_location_ids = fields.Many2many(related='current_check_id.point_id1.failure_location_ids')
    ...
    def do_pass(self):
        if self.test_type == 'picture' and not self.picture:
            raise UserError(_('Vous devez fournir une photo avant de valider'))
    ...
    def action_generate_next_window(self):
        ...
        if self._context.get('button_validate_picking_ids', False):
            self.check_ids.picking_id1.button_validate()
        return action
    ...
    def show_failure_message(self):
        ...
        return {
            'name': _('Contrôle Qualité Échoué pour %(product_name)s', product_name=self.product_id.name),
            'type': 'ir.actions.act_window',
            'res_model': 'quality.check.wizard1',
            'views': [(self.env.ref('quality_management.quality_check_wizard1_form_failure').id, 'form')],
            'target': 'new',
            'res_id': self.id,
            'context': {
                **self.env.context,
